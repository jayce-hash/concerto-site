...
SEARCH_HTML = '''
    <div class="hero-search" id="heroSearch">
      ...
      <input id="heroSearchInput" type="text" placeholder="Search your venue or tour" autocomplete="off" spellcheck="false" aria-label="Search venues and tours" role="combobox" aria-expanded="false" aria-controls="heroSearchResults">
      <div class="hero-search-results" id="heroSearchResults" role="listbox" hidden></div>
    </div>
'''
SEARCH_CSS = '''
/* ---- hero search (generated by build_static.py) ---- */
.hero-search{position:relative;max-width:520px;margin:1.6rem auto 0.4rem;display:flex;align-items:center;}
.hero-search svg{position:absolute;left:1.1rem;width:17px;height:17px;color:var(--text-xdim);pointer-events:none;}
.hero-search input{width:100%;font-family:var(--body);font-size:0.95rem;color:var(--text);background:var(--card-bg);border:1px solid var(--border);border-radius:99px;padding:0.95rem 1.4rem 0.95rem 2.9rem;box-shadow:var(--shadow-sm);outline:none;transition:border-color .2s,box-shadow .2s;}
.hero-search input:focus{border-color:var(--gold);box-shadow:var(--shadow-md);}
.hero-search input::placeholder{color:var(--text-xdim);}
.hero-search-results{position:absolute;top:calc(100% + 8px);left:0;right:0;background:var(--card-bg);border:1px solid var(--border);border-radius:18px;box-shadow:var(--shadow-md);overflow:hidden;z-index:60;text-align:left;}
.hero-search-results a{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:0.8rem 1.2rem;text-decoration:none;color:var(--text);font-family:var(--body);font-size:0.9rem;border-bottom:1px solid var(--border);}
...
.hero-search-results .hs-type{font-size:0.6rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:var(--text-xdim);flex-shrink:0;}
