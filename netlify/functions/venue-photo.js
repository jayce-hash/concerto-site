...
exports.handler = async function (event) {
  ...
  if (!key) {
    return { statusCode: 500, body: JSON.stringify({ error: 'GOOGLE_PLACES_SERVER_KEY not configured' }) };
  }
  const { name, city, lat, lng } = event.queryStringParameters || {};
  if (!name || lat === undefined || lng === undefined) {
    return { statusCode: 400, body: JSON.stringify({ error: 'name, lat, lng required' }) };
  }
  ...
  try {
    ...
    if (!searchRes.ok) {
      return { statusCode: 404, body: JSON.stringify({ error: 'search failed' }) };
    }
    ...
    if (!match) {
      return { statusCode: 404, body: JSON.stringify({ error: 'no coordinate-verified match' }) };
    }
    ...
    if (!mediaRes.ok) {
      return { statusCode: 404, body: JSON.stringify({ error: 'photo media failed' }) };
    }
