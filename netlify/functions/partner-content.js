  ...
  try {
    ...
    if (q.venue) {
      ...
      const [o, s, p, org] = await Promise.all([
        ...
        sb.from('perks').select('id,kind,partner_name,offer,details,url,address,lat,lng,venue_slugs,starts_on,ends_on').eq('status', 'live').contains('venue_slugs', [slug]),
      ...
      out.perks = (p.data || []).filter(x => (!x.starts_on || x.starts_on <= today) && (!x.ends_on || x.ends_on >= today));
    } else if (q.tour) {
      const p = await sb.from('perks').select('id,kind,partner_name,offer,details,url,tour_slug,starts_on,ends_on').eq('status', 'live').eq('tour_slug', String(q.tour).slice(0, 160));
      out.perks = (p.data || []).filter(x => (!x.starts_on || x.starts_on <= today) && (!x.ends_on || x.ends_on >= today));
    } else if (q.all) {
      const [o, p, orgs] = await Promise.all([
        sb.from('venue_overrides').select('venue_slug,section,content,verified_at'),
        sb.from('perks').select('id,kind,partner_name,offer,details,url,venue_slugs,tour_slug,starts_on,ends_on').eq('status', 'live'),
      ...
      out.perks = p.data || []; out.claimedVenues = (orgs.data || []).map(x => x.venue_slug);
    }
    return { statusCode: 200, headers: H, body: JSON.stringify(out) };
  } catch (e) {
    return { statusCode: 200, headers: { ...H, 'Cache-Control': 'no-store' }, body: JSON.stringify({ venue: q.venue || null, overrides: {}, stageTimes: [], perks: [], claimed: false, error: 'unavailable' }) };
  }
};
