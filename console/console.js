...
(function () {
  ...
  function loadCatalog() {
    return Promise.all([fetch('/data/venues.json').then(function (r) { return r.json(); }), fetch('/data/venue_info.json').then(function (r) { return r.json(); }), fetch('/data/tours.json').then(function (r) { return r.json(); })]).then(function (d) {
      ...
      $('select[name=tour_slug]').insertAdjacentHTML('beforeend', d[2].map(function (t) { return '<option value="' + esc(t.tourId) + '">' + esc(t.artist + ' · ' + t.tourName) + '</option>'; }).join(''));
    });
  }
  ...
  function renderVenue() {
    ...
    var offers = $('[data-view=partner] [data-pane=perks]'); offers.hidden = true; $('[data-view=venue]').appendChild(offers);
    show('venue'); wireTabs('venue'); wirePerks();
    ...
    $('[data-form=stage]').addEventListener('submit', function (e) {
      ...
      var row = { venue_slug: org.venue_slug, tm_event_id: f.tm_event_id.value.trim() || null, event_date: f.event_date.value, doors: f.doors.value || null, opener: f.opener.value || null, headliner: f.headliner.value || null, note: f.note.value || null, source: 'venue' };
      sb.from('stage_times').insert(row).then(function (r) { if (r.error) throw r.error; f.reset(); return sb.from('stage_times').select('*').eq('venue_slug', org.venue_slug).order('event_date'); }).then(function (r) { if (r.error) throw r.error; renderStage(r.data || []); }).catch(function () { window.alert('Could not save stage times. Please try again.'); });
    });
  }
  function renderSections(overrides) {
    ...
    $$('[data-pane=sections] form').forEach(function (f) {
      f.addEventListener('submit', function (e) {
        ...
        sb.from('venue_overrides').upsert({ venue_slug: org.venue_slug, section: key, content: content, verified_at: new Date().toISOString().slice(0, 10), updated_by: session.user.id }).then(function (r) {
          if (r.error) throw r.error;
          return sb.from('venue_overrides').select('*').eq('venue_slug', org.venue_slug);
        }).then(function (r) { if (r.error) throw r.error; renderSections(r.data || []); }).catch(function () { window.alert('Could not publish your update. Your edits are still here; please try again.'); });
      });
    });
  }
  function renderStage(rows) {
    ...
    }).join('') : '<p class="console-hint">No venue-supplied stage times yet. Only add times confirmed for this event.</p>';
  ...
  function renderPartner() {
    ...
    wirePerks(); renderReport($('[data-view=partner] [data-pane=report]'), { partner: org.name });
  }
  function wirePerks() {
    listPerks();
    $('[data-tour-field]').hidden = org.kind !== 'artist';
    if (org.kind === 'venue') $$('select[name=venue_slugs] option').forEach(function (o) { o.selected = o.value === org.venue_slug; });
    $('[data-form=perk]').addEventListener('submit', function (e) {
      ...
      var msg = $('[data-perk-status]'), button = $('button[type=submit]', f), tour = org.kind === 'artist' ? f.tour_slug.value : '';
      if (!f.offer.value.trim() || !f.details.value.trim()) { msg.textContent = 'Add an offer and complete redemption terms.'; return; }
      if (!slugs.length && !tour) { msg.textContent = 'Choose at least one venue or a tour.'; return; }
      if (f.starts_on.value > f.ends_on.value || f.ends_on.value < new Date().toISOString().slice(0, 10)) { msg.textContent = 'Choose a valid date range that has not ended.'; return; }
      if (f.url.value) { try { var u = new URL(f.url.value); if (u.protocol !== 'https:' || u.username || u.password) throw new Error(); } catch (_) { msg.textContent = 'Use a full HTTPS link without sign-in credentials.'; return; } }
      var row = { org_id: org.id, kind: org.kind, partner_name: org.name, offer: f.offer.value.trim(), details: f.details.value.trim(), url: f.url.value.trim() || null, address: f.address.value.trim() || null, venue_slugs: slugs, tour_slug: tour || null, starts_on: f.starts_on.value, ends_on: f.ends_on.value, status: 'draft' };
      button.disabled = true; msg.textContent = 'Submitting…';
      sb.from('perks').insert(row).then(function (r) { if (r.error) throw r.error; f.reset(); msg.textContent = 'Submitted for review. Your offer is not public yet.'; listPerks(); }).catch(function () { msg.textContent = 'Could not submit. Your details are still here; please try again.'; }).finally(function () { button.disabled = false; });
    });
  }
  function listPerks() {
    sb.from('perks').select('*').eq('org_id', org.id).order('created_at', { ascending: false }).then(function (r) {
      if (r.error) { $('[data-perk-list]').textContent = 'Could not load your offers. Refresh to try again.'; return; }
      var rows = r.data || [];
      $('[data-perk-list]').innerHTML = rows.length ? rows.map(function (p) {
        var state = p.status === 'draft' ? 'In review' : p.status;
        var today = new Date().toISOString().slice(0, 10);
        if (p.status === 'live' && p.ends_on && p.ends_on < today) state = 'Expired';
        else if (p.status === 'live' && p.starts_on && p.starts_on > today) state = 'Scheduled';
        return '<div class="list-row"><div><b>' + esc(p.offer) + '</b><br><span>' + esc((p.venue_slugs || []).map(function (s) { return (venues.find(function (v) { return v.id === s; }) || { name: s }).name; }).join(', ') || p.tour_slug || 'No placement selected') + '</span></div><span class="pill">' + esc(state) + '</span></div>';
      }).join('') : '<p class="console-hint">No offers submitted yet. Start with one specific benefit fans can use.</p>';
    });
  }
  ...
  function renderReport(host, filter) {
    ...
    q.order('month', { ascending: false }).limit(3).then(function (r) {
      if (r.error) { host.textContent = 'Reporting is unavailable. Please try again later.'; return; }
      ...
      host.innerHTML = '<p class="console-hint">' + (rows[0] ? 'Latest recorded month: ' + esc(String(m.month).slice(0, 7)) + '. Counts describe recorded activity, not confirmed sales or redemptions.' : 'No recorded activity yet. Counts will appear as eligible events are received.') + '</p><div class="report-grid">' + cards.map(function (c) { return '<div class="report-card"><b>' + (c[1] || 0) + '</b><span>' + c[0] + '</span></div>'; }).join('') + '</div>';
    });
  }
