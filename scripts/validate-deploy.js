...
const publicRoutes=['index','founder','venues','tours','setlists','near-me','premium','perks','partners','partner-restaurants','partner-hotels','partner-venues','partner-artists','about','investors','press','creators','contact','help','faq','search','bagcheck','bags','parking','rideshare','concessions','privacy','terms','partners-thank-you'];
...
const ruleLines=redirects.split('\n').map((l,i)=>({i,toks:l.split('#')[0].trim().split(/\s+/).join(' ')}));const catchLine=ruleLines.findIndex(r=>r.toks==='/* /404.html 404');
for(const line of ['/venues /venues.html 200','/tours /tours.html 200','/setlists /setlists.html 200','/search /search.html 200','/help /help.html 200','/venue/* /venue/:splat.html 200','/tour/* /tour/:splat.html 200','/setlist/* /setlist/:splat.html 200']){const at=ruleLines.findIndex(r=>r.toks===line);ok(at>=0&&at<catchLine,`public route rewrite invalid or after catch-all: ${line}`);}
