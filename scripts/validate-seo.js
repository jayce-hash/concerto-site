...
{const h=read('index.html');ok(h.includes('home-hero concert-hero')&&h.includes('A great show.')&&!h.includes('react-native-stylesheet'),'public marketing homepage missing');ok((h.match(/<h1[\s>]/g)||[]).length===1,'homepage must have exactly one h1');ok((h.match(/<section /g)||[]).length===5,'homepage must have five focused sections');ok((h.match(/role="tab"/g)||[]).length===3,'concert-night navigation missing');ok(!/class="(?:device|iphone-screen)/.test(h),'homepage must use unframed captures');}
...
for(const f of publicPages){const h=read(f);
  ...
  ok(!h.includes('/img/product/'),`${f}: screenshot reference is not allowed`);
