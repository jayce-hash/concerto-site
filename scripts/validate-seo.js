...
{const h=read('index.html');ok(h.includes('class="v7-hero"')&&/<h1>Everything after<br>the ticket\.<\/h1>/.test(h)&&!h.includes('react-native-stylesheet'),'public marketing homepage missing');ok((h.match(/<h1[\s>]/g)||[]).length===1,'homepage must have exactly one h1');ok(h.includes('data-venue-search')&&h.includes('data-live-ticket')&&h.includes('v7-night'),'homepage must carry the live ticket, the venue lookup, and the night timeline');ok((h.match(/class="night-card/g)||[]).length>=7,'homepage must show product content built from data');ok(!/class="(?:device|iphone-screen)|img\/product\/screens/.test(h),'no screenshots or device frames on the homepage');}
...
for(const f of publicPages){const h=read(f);
  ...
  ok(!/\{len\(|\{\{|\}\}|YOUR_|TEAMID|\bTODO\b|\bFIXME\b/.test(h.replace(/<script type="application\/ld\+json">[\s\S]*?<\/script>/g,'')),`${f}: unresolved template token in page text`);}
