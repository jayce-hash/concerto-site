...
{const h=read('index.html');ok(h.includes('class="stage-hero"')&&/<h1>Everything after the ticket,<br>in one place\.<\/h1>/.test(h)&&!h.includes('react-native-stylesheet'),'public marketing homepage missing');ok((h.match(/<h1[\s>]/g)||[]).length===1,'homepage must have exactly one h1');ok((h.match(/<section /g)||[]).length===9,'homepage must be the stage layout: hero, statement, pillars, how, moments, plus, features, proof, close');ok((h.match(/class="night-card/g)||[]).length>=8,'homepage must show product content built from data');ok(!/class="(?:device|iphone-screen)|img\/product\/screens/.test(h),'no screenshots or device frames on the homepage');}
