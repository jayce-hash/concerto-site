...
{const h=read('index.html');ok(/<h1>Everything after the ticket, in one place\.<\/h1>/.test(h)&&h.includes('class="home-hero"')&&!h.includes('react-native-stylesheet'),'public marketing homepage missing');ok((h.match(/<h1[\s>]/g)||[]).length===1,'homepage must have exactly one h1');ok((h.match(/<section /g)||[]).length===5,'homepage must be the five-section layout');}
