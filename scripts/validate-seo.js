...
{const h=read('index.html');ok(h.includes('class="c-hero"')&&/<h1>From the concert<br>to the city\.<\/h1>/.test(h)&&!h.includes('react-native-stylesheet'),'public marketing homepage missing');ok((h.match(/<h1[\s>]/g)||[]).length===1,'homepage must have exactly one h1');ok((h.match(/href="\/venue\//g)||[]).length>=10&&(h.match(/href="\/tour\//g)||[]).length>=10,'homepage must link into the venue and tour guides');ok(!/class="(?:device|iphone-screen|night-card)|img\/product\/screens/.test(h),'no screenshots, device frames, or imitation app UI on the homepage');}
